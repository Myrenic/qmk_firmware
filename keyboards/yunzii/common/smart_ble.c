...
static void ble_queue_push(report_type_t type, const void *data, uint8_t len) {
    ...
    if (next == ble_queue_tail) {
        // Queue full — drop oldest to prevent stale data buildup
...
void smart_ble_task(void) {
    // 1. Process incoming UART (simple parser matching vendor format)
    uint8_t max_reads = 32; // Prevent excessive loop time
    while (uart_available() && max_reads--) {
        uint8_t c = uart_read();
        ...
        switch (uart_state) {
            ...
            case UART_0X55_RECEIVED:
                if (c == 0x55) break; // Skip duplicate start bytes
                uart_lens = c;
                uart_command[uart_buff_index++] = c;
    ...
    // 3. Process outgoing report queue
    if (ble_queue_head != ble_queue_tail) {
        ...
        if (timer_elapsed32(last_send_time) >= delay) {
            ...
            // Wake module if it hasn't received data recently
...
// Send keepalive bytes to wireless MCU (called from keypress handler)
...
// Send an empty keyboard report to clear any stuck keys on the wireless MCU
void smart_ble_send_clear(void) {
    if (!wireless_connected) return;
    uint8_t empty_report[KEYBOARD_REPORT_SIZE];
    memset(empty_report, 0, sizeof(empty_report));
    ble_queue_push(REPORT_KEYBOARD, empty_report, KEYBOARD_REPORT_SIZE);
}
...
static void sc_send_nkro(report_nkro_t *report) {
    if (!wireless_connected) return;
    ble_queue_push(REPORT_NKRO, report, WIRELESS_NKRO_REPORT_LEN);
}
